...
export default class FacturiEmiseController {
    ...
    public async printfactura({view,params}){
       ...
       let sql=`
       SELECT fe.id id,furnizor.denumire denumirefurnizor,furnizor.alias alias,furnizor.cui codfiscalfurnizor,furnizor.banca banca,produse.iban iban,cl.denumire numeclient,cl.codfiscal codfiscalclient,cl.adresa adresaclient,cl.judet judetclient, DATE_FORMAT(fe.dataFactura, '%d.%m.%Y') AS dataFactura,fe.scadenta scadenta,fe.serie serie, fe.numar numar ,fe.intocmit intocmit,fe.cnp cnp, fe.mentiuni mentiuni,produse.denumire numeprodus,liniife.descriere descriere,liniife.um um, liniife.cantitate cantitate,liniife.pret pret, liniife.valoare valoare FROM cdata.facturi_emise fe
      ...
      return new Promise((resolve, reject) => {
       ...
       //   console.log('File has been successfully saved')
        ...
        // return {stare:'ok'}
         ...
         fs.rmSync('./extrase', { recursive: true })
        })
